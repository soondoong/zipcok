...
public class SmsSend {
	 ...
	 */
	  ...
	  public void sendCertifiedNumber(String phoneNumber, String cerNum) {

	    String api_key = "NCSULEM5CE879MUF";   //발급받은 API 키쓰기
	    String api_secret = "LBAJJXR0A4OUWSBSTKNRGPQ5EIHMYFJO";
	    Message coolsms = new Message(api_key, api_secret);

	    // 4 params(to, from, type, text) are mandatory. must be filled
	    HashMap<String, String> params = new HashMap<String, String>();
	    params.put("to", phoneNumber); // 수신번호
	    params.put("from", "01075250464"); // 발신번호
	    params.put("type", "SMS"); // Message type ( SMS, LMS, MMS, ATA )
	    params.put("text","집콕헬스 본인인증 : 인증번호는" + "["+cerNum+"]" + "입니다."); // 문자내용    
